...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ─── ReCode Local Analyzer (your custom logic) ───
    function scanSourceCode(code) {
        ...
        if ((code.match(/for/g) || []).length > 1) {
            suggestions.push("⚠️ Multiple loops detected. Try optimizing.");
        }
        ...
        if (code.length > 300) {
            suggestions.push("📏 Code is too long. Consider modularizing.");
        }

        let score = 10;
        if (code.includes("var ")) score -= 2;
        if ((code.match(/for/g) || []).length > 1) score -= 2;
        if (code.length > 300) score -= 2;
...
Analyze the given code and return ONLY:

Line 1: Time Complexity in Big-O notation  
Line 2: Space Complexity in Big-O notation  
Line 3 (optional): Stack Space Complexity if recursion is used  

STRICT RULES:
- No explanations
- No labels
- No markdown
- Output only raw values like:
O(n log n)
O(n)
Stack Space: O(n)
...
${code}`;
            case 'explainComplexity':
                return `You are a top FAANG interviewer.
...
## Key Observations
- Mention loops, recursion, data structures
- Max 4 bullets

## Optimization Insight
- Can this be improved? If yes, how (1 line)
- If optimal → say "Already optimal"

Keep it sharp and technical.
...
- Never write code. Never give the answer outright.`;
    ...
    async function callGeminiApi(code, problemContext, promptType, activeButton) {
        if (!geminiApiKey) {
            console.log("[ReCode] No API key found, skipping API call.");
            return;
        }
        ...
        try {
            ...
            // Enhance: add typing-effect class for better UX
            resultsEl.className = 'result-content fade-in typing-effect';
            ...
            if (data.candidates && data.candidates[0]?.content?.parts?.[0]?.text) {
                ...
                if (promptType === 'complexityOnly') {
                    ...
                    // 🔥 ReCode Hybrid Suggestions (local + AI)
                    const localResult = scanSourceCode(code);

                    if (localResult.suggestions.length > 0) {
                        let suggestionHTML = "<h3>⚡ ReCode Suggestions</h3><ul>";

                        localResult.suggestions.forEach(s => {
                            suggestionHTML += `<li>${s}</li>`;
                        });

                        suggestionHTML += "</ul>";

                        const scoreHTML = `<h3>📊 Code Score: ${localResult.score}/10</h3>`;
  ...
  `;
                    ...
                    // Apply typing effect (raw text for smooth animation)
                    typeText(resultsEl, rawText);
                }
                scrollToResults();
    ...
    getComplexityOnlyButton.addEventListener('click', async () => {
        ...
        if (!code) {
            resultsEl.innerHTML = "<p>No code found for analysis.</p>";
            return;
        }

        // Call AI for complexity (RESTORED)
